...
using ARenderer.Gl;
using ARenderer.GlAbstractions;
...
public sealed class Renderer : IDisposable
{
	...
	public unsafe Renderer(IGlApi gl)
	{
		this.gl = gl;
	...
	public void UpdateViewportSize(uint width, uint height)
	{
	...
	public void BatchCommit()
	{
		...
		gl.BindBuffer(BufferTarget.Array, vbo.Id);
		gl.BufferSubData<Vertex>(BufferTarget.Array, 0, vertices.AsSpan()[..(batchQuadCount * verticesPerQuad)]);
		gl.BindBuffer(BufferTarget.Array, 0);
	...
	private int BatchPushTexture(Texture texture)
	{
		...
		onDisposing = (_, _) =>
		{
			...
			gl.DeleteTexture(knownTextures[texture]);
			knownTextures.Remove(texture);
			dirtyTextures.Remove(texture);
		};
	...
	public void DrawTexture(Texture texture, Vector2 position, Vector2 size)
	{
		...
		BatchPushQuad(
			new(position, Color.White, new(0.0f, 0.0f), texId + 1),
			new(position + size with { Y = 0 }, Color.White, new(1.0f, 0.0f), texId + 1),
			new(position + size, Color.White, new(1.0f, 1.0f), texId + 1),
			new(position + size with { X = 0 }, Color.White, new(0.0f, 1.0f), texId + 1)
		);
	}
